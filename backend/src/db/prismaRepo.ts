...
const toModel = (row: Awaited<ReturnType<typeof prisma.scheduledEmail.findMany>>[number]): ScheduledEmail => ({
  id: row.id,
  userId: row.userId,
  subject: row.subject,
  body: row.body,
...
export const prismaRepo = {
  ...
  async incrementFailed(id: string, count: number, error: string): Promise<void> {
    await prisma.scheduledEmail.update({
      where: { id },
      data: { failCount: { increment: count }, errorMessage: error, status: 'FAILED' },
    });
  },
