...
async function request<T>(path: string, options?: RequestInit): Promise<T> {
  ...
  const token = activeId ? localStorage.getItem(`reachinbox.token.${activeId}`) : null;
...
export const api = {
  login: (credential: string) => request<{ user: AuthUser; token: string }>('/auth/google', { method: 'POST', body: JSON.stringify({ credential }) }),
  me: () => request<{ user: AuthUser }>('/auth/me'),
  scheduleEmail: (input: ScheduleEmailInput) =>
    request<ScheduledEmail>('/schedule-email', { method: 'POST', body: JSON.stringify(input) }),
  listEmails: () => request<ScheduledEmail[]>('/emails'),
  getEmail: (id: string) => request<ScheduledEmail>(`/emails/${id}`),
  deleteEmail: (id: string) => request<{ success: boolean }>(`/emails/${id}`, { method: 'DELETE' }),
